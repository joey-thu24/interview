...
def get_today_plan(db: Session):
    return db.query(StudyPlan).filter(StudyPlan.date == date.today()).first()

def create_daily_plan(db: Session, plan_data: dict, encouragement: str):
...
def create_interview_session(db: Session, topic: str):
    session = InterviewSession(topic=topic, messages=[])
    db.add(session)
    db.commit()
    db.refresh(session)
    return session
...
def get_recent_weaknesses(db: Session, limit: int = 3):
    """
...
def get_all_finished_sessions(db: Session):
    """获取所有已完成评价的面试"""
    return db.query(InterviewSession).filter(InterviewSession.score.isnot(None)).all()

def get_study_stats(db: Session):
    """计算学习任务完成率"""
    ...
    finished_sessions_count = db.query(InterviewSession).filter(InterviewSession.score.isnot(None)).count()
